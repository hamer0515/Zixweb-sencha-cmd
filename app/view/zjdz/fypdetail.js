Ext.define('Zixweb.view.zjdz.fypdetail', {
	extend : 'Ext.panel.Panel',
	alias : 'widget.zjdzfypdetail',
	border : false,
	...
	initComponent : function() {
		...
		var tpl = new Ext.XTemplate(
				'<tpl for=".">'	,
				'<table width="98%" id="zjdzfyp_{fyp_acct}_{zjbd_date}" border="0" cellspacing="1" cellpadding="0" align="center"  bgcolor="#C8DCF0" class="live_1_table">',
				'<tr align="center" class="ice_one_td" >',
				'<td width="15%" colspan="1">',
				'电话充值业务</td>',
				'<td width="15%" colspan="1">',
				'资金变动日期： {zjbd_date}</td>',
				'<td width="15%" colspan="1">易宝中间账户号：',
				'{acct_name}</td><td ></td></tr>',
				...
				'{[this.gentrs(values)]}', '</table>', '</tpl>', { 
					gentrs : function(json) {
						...
						for (var i = 0; i < json.l; i++) {
							...
							data += '<td rowspan="1" width="15%" >预付账款-易宝结算账户</td>';
		...
		var store = new Ext.data.Store({
			fields : ['fyp_acct', 'acct_name', 'zjbd_date', 'l', 
					'real_ch_amt', 't_ids', 'data', 'ch_fyp'],
			proxy : {
				type : 'ajax',
				url : 'zjdz/fypcheck'
			},
				...
				load : function(me, records, successful, eOpts) {
					...
					if (bookdetailButton) {
						bookdetailButton.on('click', function(e, btn, eOpts) {
							var bfj_id = records[0].data.fyp_acct;
							var viewport = fypdetail.up('viewport'), center = viewport
									.down('center'), id = 'book_hist_yp_acct_fhyd',   
								    cmp = Ext.getCmp(id), panel;
							if (!cmp) {
								cmp = Ext.widget('book_hist_yp_acct_fhyd');
								panel = center.add({
											closable : true,
											xtype : 'panel',
											items : cmp,
							...
							cmp.down("fypacct[name='fyp_acct']").setValue(records[0].data.fyp_acct);
					...
					// input注册事件
					var id = 'zjdzfyp_' + records[0].data.fyp_acct + '_'
					...
					if (checkbtn) {
						checkbtn.on('click', function(e, btn, eOpts) {
							var id = 'zjdzfyp_' + records[0].data.fyp_acct + '_' + records[0].data.zjbd_date;
							var tbl = Ext.get(id);
							var fields = tbl.select("input[type=text]");
							var mydata = {};
							for (var i in fields.elements) {
							...
							mydata.fyp_acct = records[0].data.fyp_acct;
							store.load({
										params : mydata
									});
							}, self);
					...
					if (checkdonebtn) {
						checkdonebtn.on('click', function(e, btn, eOpts) {
							var real_ch_amt = Ext.get('real_ch_amt')
									.getValue().replace(',', '', 'g');
							var ch_pre_amt = records[0].data.data[records[0].data.ch_fyp[3]]
									.replace(',', '', 'g');
							var lc_amt = records[0].data.data[records[0].data.ch_fyp[5]]
									.replace(',', '', 'g');
							var sc_amt = records[0].data.data[records[0].data.ch_fyp[6]]
									.replace(',', '', 'g');
							...
							Ext.MessageBox.confirm('提示', '确定进行[对账成功]操作？',
									function(opt) {
										if (opt === 'yes') {
											...
											data.fyp_acct = records[0].data.fyp_acct;
											Ext.Ajax.request({
												async : false,
												url : 'zjdz/fypcheckdone',
												params : data,
