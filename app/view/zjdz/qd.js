		...
		grid = new Ext.grid.Panel({
			...
			columns : [{
				...
				renderer : function(value, p, record) {
					var f_ch = Ext.data.StoreManager.lookup('component.Fch');
					...
					handler : function(grid, rowIndex, colIndex) {
						...
						if (cmp) {
							...
							var fch = Ext.data.StoreManager
									.lookup('component.Fch');
							var index = fch.findExact('id', rec.data.fch);
							var fch_name = fch.getAt(index).data.name;
							...
							center.add({
								...
								items : zjdzqddetail,
								id : id,
								title : fch_name + '渠道编号' + rec.data.zjdz_date
										+ '日资金对账'
							}).show();
						}
					}
