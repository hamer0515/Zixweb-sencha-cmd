Ext.define('Zixweb.view.fhydbook.detail.ckrsp_fhyd', {
	extend : 'Ext.panel.Panel',
	alias : 'widget.book_detail_ckrsp_fhyd',
	...
	initComponent : function() {
		var store = new Ext.data.Store({
					fields : ['f_ssn', 'fs_rate', 'fhw_type', 'fyw_type', 'ftx_date', 'period', 'j', 'd'],
					...
					listeners : {
						beforeload : function(store, operation, eOpts) {
							var form = Ext.getCmp('ckrspfhyddetailform')
									.getForm();
							var values = form.getValues();
							var grid = Ext
									.getCmp('book_detail_ckrsp_fhyd_grid');
						...
						load : function(thiz, records, successful, eOpts) {
							if (!successful) {
								Ext.MessageBox.show({
											title : '警告',
											msg : '应付账款-持卡人-商品科目详细数据加载失败,请联系管理员',
											buttons : Ext.Msg.YES,
											icon : Ext.Msg.ERROR
										});
								return;
							...
							if (jsonData && jsonData === 'forbidden') {
								Ext.MessageBox.show({
											title : '警告',
											msg : '抱歉，没有应付账款-持卡人-商品科目详细数据访问权限',
											buttons : Ext.Msg.YES,
											icon : Ext.Msg.ERROR
										});
							}
		...
		this.items = [{
					...
					items : [{
								...
								items : [{
											...
											margin : '0 10 0 0',
											verify : {
												id : 'book_detail_ckrsp_fhyd_to_2'
											},
											vtype : 'dateinterval',
											width : 180
										}, {
					...
					xtype : 'gridpanel',
					id : 'book_detail_ckrsp_fhyd_grid',
					height : 'auto',
					store : this.store,
					dockedItems : [{
								xtype : 'pagingtoolbar',
