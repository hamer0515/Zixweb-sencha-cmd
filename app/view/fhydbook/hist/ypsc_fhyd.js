Ext.define('Zixweb.view.fhydbook.hist.ypsc_fhyd', {
	extend : 'Ext.panel.Panel',
	alias : 'widget.book_hist_ypsc_fhyd',
	...
	initComponent : function() {
		var store = new Ext.data.Store({
					fields : ['id', 'fyw_type', 'fyp_acct', 'fe_date', 'period', 'j', 'd',
							'ys_id', 'ys_type'],
					...
					listeners : {
						beforeload : function(store, operation, eOpts) {
							var form = Ext.getCmp('ypscfhydform')
									.getForm();
							var values = form.getValues();
							var grid = Ext.getCmp('book_hist_ypsc_fhyd_grid');
							if (form.isValid()) {
						...
						listeners : {
							load : function(thiz, records, successful, eOpts) {
								if (!successful) {
									Ext.MessageBox.show({
												title : '警告',
												msg : '应收账款-易宝-短款科目历史数据加载失败,请联系管理员',
												buttons : Ext.Msg.YES,
												icon : Ext.Msg.ERROR
											});
									return;
