Ext.define('Zixweb.view.fhydbook.hist.yplc_fhyd', {
	extend : 'Ext.panel.Panel',
	alias : 'widget.book_hist_yplc_fhyd',
	...
	initComponent : function() {
		var store = new Ext.data.Store({
					fields : ['id', 'fyw_type',  'fe_date', 'period','fyp_acct','j', 'd',
							'ys_id', 'ys_type'],
					...
					listeners : {
						beforeload : function(store, operation, eOpts) {
							var form = Ext.getCmp('yplcfhydform')
									.getForm();
							var values = form.getValues();
							var grid = Ext.getCmp('book_hist_yplc_fhyd_grid');
							if (form.isValid()) {
						...
						listeners : {
							load : function(thiz, records, successful, eOpts) {
								if (!successful) {
									Ext.MessageBox.show({
												title : '警告',
												msg : '应付账款-持卡人-商品科目历史数据加载失败,请联系管理员',
												buttons : Ext.Msg.YES,
												icon : Ext.Msg.ERROR
											});
									return;
								...
								if (jsonData && jsonData === 'forbidden') {
									Ext.MessageBox.show({
												title : '警告',
												msg : '抱歉，没有应付账款-持卡人-商品科目历史数据访问权限',
												buttons : Ext.Msg.YES,
												icon : Ext.Msg.ERROR
											});
								}
