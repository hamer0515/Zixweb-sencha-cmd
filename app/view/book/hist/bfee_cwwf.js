Ext.define('Zixweb.view.book.hist.bfee_cwwf', {
	...
	initComponent : function() {
		var store = new Ext.data.Store({
			fields : ['id', 'bi', 'period', 'tx_date','j', 'd', 'ys_id', 'ys_type'],
			...
			listeners : {
				beforeload : function(store, operation, eOpts) {
					var form = Ext.getCmp('bfeecwwfhistform').getForm();
					var values = form.getValues();
					var grid = Ext.getCmp('book_hist_bfee_cwwf_grid');
					if (form.isValid()) {
				...
				listeners : {
					load : function(thiz, records, successful, eOpts) {
						if (!successful) {
							Ext.MessageBox.show({
										title : '警告',
										msg : '财务外付银行手续费科目历史数据加载失败,请联系管理员',
										buttons : Ext.Msg.YES,
										icon : Ext.Msg.ERROR
									});
							return;
