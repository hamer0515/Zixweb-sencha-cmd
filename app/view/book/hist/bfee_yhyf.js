		...
		var store = new Ext.data.Store({
					...
					listeners : {
						beforeload : function(store, operation, eOpts) {
							...
							var grid = Ext.getCmp('book_hist_bfee_yhyf_grid');
							if (form.isValid()) {
						...
						listeners : {
							load : function(thiz, records, successful, eOpts) {
								if (!successful) {
									Ext.MessageBox.show({
												title : '警告',
												msg : '已核应付银行手续费科目历史数据加载失败,请联系管理员',
												buttons : Ext.Msg.YES,
												icon : Ext.Msg.ERROR
											});
									return;
