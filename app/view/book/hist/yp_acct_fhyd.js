Ext.define('Zixweb.view.book.hist.yp_acct_fhyd', {
	...
	initComponent : function() {
		var store = new Ext.data.Store({
					...
					listeners : {
						beforeload : function(store, operation, eOpts) {
							var form = Ext.getCmp('ypacctfhyd form')
									.getForm();
							var values = form.getValues();
							var grid = Ext.getCmp('book_hist_yp_acct_fhyd_grid');
							if (form.isValid()) {
						...
						listeners : {
							load : function(thiz, records, successful, eOpts) {
								if (!successful) {
									Ext.MessageBox.show({
												title : '警告',
												msg : '预付账款-易宝结算账户科目历史数据加载失败,请联系管理员',
												buttons : Ext.Msg.YES,
												icon : Ext.Msg.ERROR
											});
									return;
								...
								if (jsonData && jsonData === 'forbidden') {
									Ext.MessageBox.show({
												title : '警告',
												msg : '抱歉，没有预付账款-易宝结算账户科目历史数据访问权限',
												buttons : Ext.Msg.YES,
												icon : Ext.Msg.ERROR
											});
								}
		...
		this.items = [{
					...
					xtype : 'gridpanel',
					id : 'book_hist_yp_acct_fhyd_grid',
					height : 'auto',
