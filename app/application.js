		...
		Ext.apply(Ext, {
			columns : {
				...
				flag : { // 凭证状态（原始凭证查询）
					text : "撤销状态",
					dataIndex : 'flag',
					renderer : Ext.data.StoreManager
							.lookup('component.RStatus').$render,
					flex : 1
				},
				acct : {
					text : "银行账户号",
				...
				p : {
					...
					dataIndex : 'p',
					renderer : Ext.data.StoreManager
							.lookup('component.Product').$render,
					flex : 1
				},
				/*
				 * 日期类
				...
				zyzj_acct : {
					...
					dataIndex : 'zyzj_acct',
					renderer : Ext.data.StoreManager
							.lookup('component.ZyzjAcct').$render,
					flex : 1
				},
				wlzj_type : {
					text : "往来类型",
					dataIndex : 'wlzj_type',
					renderer : Ext.data.StoreManager
							.lookup('component.WlzjType').$render,
					flex : 1
				},
				bfj_acct : {
					text : "备付金帐号",
					dataIndex : 'bfj_acct',
					renderer : Ext.data.StoreManager
							.lookup('component.BfjAcct').$render,
					flex : 1
				},
				zjbd_type : {
					text : "资金变动类型",
					dataIndex : 'zjbd_type',
					renderer : Ext.data.StoreManager
							.lookup('component.ZjbdType').$render,
					flex : 1
				},
				fyw_type : {
					text : "业务类型",
					dataIndex : "fyw_type",
					renderer : Ext.data.StoreManager
							.lookup('component.FywType').$render,
					flex : 1
				},
				fhyd_acct : {
					text : "富汇易达账号",
					dataIndex : 'fhyd_acct',
					renderer : Ext.data.StoreManager
							.lookup('component.FhydAcct').$render,
					flex : 1
				},
				fyp_acct : {
					text : "富汇易达账号",
					dataIndex : 'fyp_acct',
					renderer : Ext.data.StoreManager
							.lookup('component.FypAcct').$render,
					flex : 1
				},
				fhw_type : {
					text : "货物类型",
					dataIndex : 'fhw_type',
					renderer : Ext.data.StoreManager
							.lookup('component.FhwType').$render,
					flex : 1
				},
				j : {
					text : "借方金额",
